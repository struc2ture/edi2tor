...
void on_render(Editor_State *state, const Platform_Timing *t)
{
    ...
    draw_grid((Vec_2){state->canvas_viewport.rect.x, state->canvas_viewport.rect.y}, &state->render_state);
...
{
    Rect cursor_rect = draw_string_font_get_string_char_rect(text_buffer.lines[cursor_pos.line].str, render_state->font, cursor_pos.col);
    float line_height = draw_string_font_get_line_height(render_state->font);
    float x = 0;
...
//     {
//         Rect string_rect = draw_string_font_get_string_rect(text_buffer.lines[i].str, render_state->font, 0, y);
...
//                 selected_rect.y += i * draw_string_font_get_line_height(render_state->font);
...
//     const float font_line_height = draw_string_font_get_line_height(render_state->font);
...
void draw_image_view(Image_View *image_view, Render_State *render_state)
{
    ...
    draw_textured_quad(image_view->image.texture, image_view->image_rect, (Color){255, 255, 255, 255}, render_state);
...
void draw_live_scene_view(Live_Scene_View *ls_view, Render_State *render_state, const Platform_Timing *t)
{
    ...
    draw_textured_quad(ls_view->framebuffer.tex, ls_view->framebuffer_rect, (Color){255, 255, 255, 255}, render_state);
    glUseProgram(render_state->main_shader);
}

Mat_4 viewport_get_transform(Viewport viewport)
{
    Mat_4 scale = mat4_scale(viewport.zoom, viewport.zoom, 1);
    Mat_4 translate = mat4_translate(viewport.rect.x, viewport.rect.y, 0);
    Mat_4 transform = mat4_mul(translate, scale);
    return transform;
}
...
Rect_Bounds get_viewport_cursor_bounds(Viewport viewport, Render_Font font)
{
    ...
    float space_width = draw_string_font_get_char_width(' ', font);
    float font_line_height = draw_string_font_get_line_height(font);
...
{
    Cursor_Pos cursor;
    cursor.line = buffer_pos.y / (float)draw_string_font_get_line_height(render_state->font);
    if (cursor.line < 0) {
        cursor.line = 0;
    } else if (cursor.line >= text_buffer.line_count) {
        cursor.line = text_buffer.line_count - 1;
    }
    cursor.col = draw_string_font_get_char_i_at_pos_in_string(text_buffer.lines[cursor.line].str, render_state->font, buffer_pos.x);
    return cursor;
}
...
void viewport_snap_to_cursor(Text_Buffer text_buffer, Cursor_Pos cursor_pos, Viewport *viewport, Render_State *render_state)
{
    ...
    float font_space_width = draw_string_font_get_char_width(' ', render_state->font);
    float font_line_height = draw_string_font_get_line_height(render_state->font);
